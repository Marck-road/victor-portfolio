...
const Home: React.FC = () => {
  ...
  // Text Parallax: Slides down slightly and fades out
  const textY = useTransform(scrollY, [0, 400], [0, 80]);
  const textOpacity = useTransform(scrollY, [0, 350], [1, 0]);

  // Image Parallax: Slides up and right while fading out
  const imageY = useTransform(scrollY, [0, 500], [0, -120]);
  const imageX = useTransform(scrollY, [0, 500], [0, 60]);
  const imageOpacity = useTransform(scrollY, [0, 450], [1, 0]);

  // Floating Badge Parallax: Moves independently for depth
  ...
  return (
    <section className="bg-body min-h-screen flex items-center pt-24 pb-12 px-4 relative overflow-hidden" id="home">
      {/* Background ambient glow that also fades on scroll */}
      <MotionDiv 
        style={{ opacity: imageOpacity } as any}
        className="absolute inset-0 z-0"
      >
        <div className="absolute top-[20%] left-[10%] w-[400px] h-[400px] bg-first/10 blur-[120px] rounded-full"></div>
        <div className="absolute bottom-[20%] right-[10%] w-[300px] h-[300px] bg-blue-500/10 blur-[100px] rounded-full"></div>
      </MotionDiv>
      ...
      <div className="max-w-[1120px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center relative z-10">
        ...
        <div className="flex justify-center order-1 lg:order-2 relative">
          <MotionDiv
            style={{ y: imageY, x: imageX, opacity: imageOpacity } as any}
          ...
          >
            ...
            <svg className="home__blob w-[320px] lg:w-[520px] fill-first relative z-10 drop-shadow-2xl" viewBox="0 0 550 591" xmlns="http://www.w3.org/2000/svg">
